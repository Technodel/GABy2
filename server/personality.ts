/**
 * GABy Personality Module
 *
 * Loads gaby_status_messages.txt (project root) once at startup and serves
...
function loadBank(): MessageBank {
  ...
  const candidates = [
    path.join(process.cwd(), 'gaby_status_messages.txt'),
    path.join(__dirname, '../../gaby_status_messages.txt'),
    path.join(__dirname, '../../../gaby_status_messages.txt'),
  ];
  ...
  if (!raw) {
    console.warn('[personality] gaby_status_messages.txt not found — using fallbacks');
    bank = {};
    return bank;
  }
