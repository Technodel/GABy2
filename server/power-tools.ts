/**
 * GABy Power Tools -- adapted from the original codebase for GABy's bridge architecture.
 *
...
export function createPowerTools(ctx: PowerToolContext): ToolSet {
  ...
  const fileReadTool = tool({
    ...
    execute: async (input) => {
      ...
      userClientManager.pushNarration(userId, `Reading ${path.basename(input.filePath)}...`);
      try {
...
Include enough context to uniquely identify the location. Do not use escape characters.`,
    ...
    execute: async (input) => {
      ...
      const abs = resolvePath(input.filePath, projectPath);
      userClientManager.pushNarration(userId, `Editing ${path.basename(input.filePath)}...`);
...
Modes: 'create_only' (fail if exists), 'overwrite' (replace or create), 'append' (add to end or create).`,
    ...
    execute: async (input) => {
      ...
      const abs = resolvePath(input.filePath, projectPath);
      userClientManager.pushNarration(userId, `Writing ${path.basename(input.filePath)}...`);
      try {
  ...
  const bashTool = tool({
    ...
    execute: async (input) => {
      ...
      userClientManager.pushNarration(userId, `Running: ${input.command.slice(0, 60)}...`);
      try {
