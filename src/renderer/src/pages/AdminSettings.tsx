...
export default function AdminSettings() {
  ...
  async function saveSettings() {
    const dailyLimitValue = settings.daily_token_limit.trim() === '' ? null : parseInt(settings.daily_token_limit, 10);
    const res = await fetch('/admin/api/settings', {
      ...
      body: JSON.stringify({
        ...settings,
        daily_token_limit: dailyLimitValue !== null && !isNaN(dailyLimitValue) ? dailyLimitValue : null,
      }),
    });
    if (res.ok) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 20, fontWeight: 600, marginBottom: 24 }}>⚙️ Settings</h1>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        {[
          ...
          { key: 'prompt_caching_enabled' as const, label: '🧠 Prompt Caching', desc: 'Caches static context (system prompt, project map, open files) to cut input costs by up to 90% on repeat turns. Works with all supported models.' },
        ].map(s => (
      ...
      {/* Daily Token Limit */}
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontWeight: 600, marginBottom: 4 }}>Daily Token Limit</h3>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
          Set the maximum total input + output tokens SUNy can use per user per day. Leave blank for unlimited. When the limit is reached, SUNy stays usable in talk-only free mode.
        </p>
        <input
          type="number"
          min={0}
          ...
          onChange={e => setSettings(prev => ({ ...prev, daily_token_limit: e.target.value }))}
      ...
      {/* Edit Format */}
      <div className="card" style={{ marginBottom: 20 }}>
        <h3 style={{ fontWeight: 600, marginBottom: 4 }}>✏️ Edit Format</h3>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
          Controls how SUNy makes changes to your code files. Choose the strategy that best matches your subscription plan and the size of your project.
        </p>

        {([
          { value: 'tool-call', label: '🔧 Smart Edit (Recommended)', desc: 'SUNy reads and edits only the exact parts of files it needs to change. Most accurate, lowest token usage — changes are precise and safe.' },
          { value: 'diff', label: '🧩 Patch Mode', desc: 'SUNy describes the exact lines to find and replace in each file. Great for plans with limited tokens — uses slightly fewer tokens than Smart Edit, works well on all plan levels.' },
          { value: 'whole', label: '📋 Full Rewrite', desc: 'SUNy rewrites the complete file from scratch each time. Simple and straightforward, but uses the most tokens. Best for small files or quick prototypes.' },
          { value: 'architect', label: '🏗️ Architect (Best Quality)', desc: 'Two-step process: SUNy first thinks through the full plan, then executes every change carefully. Highest quality for large or complex projects — uses roughly 2× tokens per task.' },
        ...
        <div style={{ marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
          <button className="btn btn-primary" onClick={saveSettings}>
            {settingsSaved ? '✓ Saved!' : 'Save Edit Format'}
          </button>
        </div>
      </div>
      {/* Change Admin Password */}
      <div className="card">
        <h3 style={{ fontWeight: 600, marginBottom: 16 }}>🔐 Change Admin Password</h3>
