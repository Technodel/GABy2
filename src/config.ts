...
const CONFIG_DIR = path.join(os.homedir(), '.gaby');
const CONFIG_FILE = path.join(CONFIG_DIR, 'config.json');

interface GabyConfig {
  token?: string;
  server?: string;
  registered_paths?: string[];
}

export function readConfig(): GabyConfig {
  try {
...
export function writeConfig(config: GabyConfig): void {
...
export function updateConfig(updates: Partial<GabyConfig>): void {
